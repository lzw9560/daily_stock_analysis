...
> 说明：当前默认 `BACKTEST_ENGINE_VERSION=v1`，回测服务会优先走 `VbtEngine` 的批量前向日线预取路径，再对每条历史分析做单条评估；未来如需切换到其他引擎版本，可继续沿用同一开关做 opt-in 分流。
...
`POST /api/v1/backtest/optimize` 会沿用 `BACKTEST_ENGINE_VERSION=v1` 的 `VbtEngine` 批量路径，对默认 TA 参数网格做 5000 组合扫描，并返回最优组合与全部候选结果。该接口当前仅用于实验性优化，不影响默认回测结果写入。

优化结果会同步写入 `backtest_optimization_logs`，便于后续回查最近一次扫描的最优参数、评分和完整候选结果。

Web 回测页会显示最近一次优化扫描摘要，并可直接重新触发扫描；页面读取的是同一份优化日志接口，不会改变常规回测结果列表。

展开后的优化卡片会展示最优参数、最佳结果与最近 5 次扫描历史，便于直接比对不同配置的评分变化。
历史表里的“查看候选”会打开抽屉，展示该次扫描的完整候选结果列表，方便进一步比较各参数组合。
